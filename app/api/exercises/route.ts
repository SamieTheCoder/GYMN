...
export async function GET(request: Request) {
    ...
    try {
        ...
        // For each query parameter key
        for (const key of queryParams) {
            if (key === "muscles" || key === "equipment" || key === "level") {
                // Filter
                const value = searchParams.get(key); // Get the value, if not null and not empty
                if (value !== null && value !== "") {
                    query = query.filter(key, "cs", `{${value}}`); // Apply filter
                }
            }
        }

        let { data, error } = await query; // Execute query
